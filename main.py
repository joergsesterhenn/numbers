...
def main():
    ...
    while user_input not in ('q','Q'):
        while user_input not in ('q', 'Q', 'g','G','e','E'):
            user_input = input("Enter language [e: english, g: german (q to quit)]: ")
            lang = user_input
        ...
        while user_input not in ('q', 'Q'):
            ...
            if number.isdecimal():
                if lang in ('g','G'):
                    print_and_say(GermanNumberWriter(int(number)).to_text(),
                                  'de')

                else:
                    print_and_say(EnglishNumberWriter(int(number)).to_text())
